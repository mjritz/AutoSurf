#ads in structure of surfactant and outputs xyz file, bond information, and
#modifies system_conf.lt file for running moltemplate
...
#Read in the monomer structure and input file for moltemplate 
in_structure = open(sys.argv[1], 'r') 
out_structure = open('monomer.xyz', 'wa')
packmol_file = open('monomer_surface.inp', 'wa')

with in_structure as f:                                                                                  
    lines = f.readlines()                                                                                
    num_lines = len([l for l in lines if l.strip(' \n') != ''])   

variables.read_variables('input/input_file.txt')
...
for lines in in_structure_again.readlines():
  if 'B' in lines or 'T' in lines or 'H' in lines or 'E' in lines:
    atoms_on_each_line = len(lines.split())
    number_of_atoms += atoms_on_each_line
  for x in lines.split():                                                                                 
    if 'T'in x:                                                                                         
      main_branch += "NP         %9.5f         %9.5f          %9.5f\n" %(zero,zero,i)                                     
      i+=float(bead_dist)       
    if x == 'H':
      main_branch += "P         %9.5f         %9.5f          %9.5f\n" %(zero,zero,i)  
      i+= float(bead_dist)
    if x == 'E':
      main_branch += "TP         %9.5f         %9.5f          %9.5f\n" %(zero,zero,i) 
      i+= float(bead_dist)	
  if lines.count('B')==0:
    pass
  if lines.count('B') > 0:
...
in_lt_file = open('system_conf.lt', 'w')
